...
char *nom,*mdp,*site,*code,*nomlong,*nomcourt;
time_t tim;
struct tm *TM;
...
nomlong=(char*)malloc(query_size);
nomcourt=(char*)malloc(query_size);
res=(char*)malloc(100);
...
tamb(code);
get_chaine(envoi,2,nomlong);
tamb(nomlong);
get_chaine(envoi,1,nomcourt);
tamb(nomcourt);
succes=0;
	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
         	...
         	if(succes==1)
         		{
         		sprintf(query,"select * from categorie where nomcourt=\'%s\' and site=\'%s\';",nomcourt,site);
         		db_query(handler,query);
         		if(db_ntuples(result)!=0)
         			{
         			db_clear_result(result);
         			succes=0;
         			strcpy(reponse,"Cette catégorie existe déjà ...");
         			}
         		else
         			{
         			db_clear_result(result);
         			sprintf(query,"insert into categorie values (\'%s\',\'%s\',\'%s\');",nomlong,nomcourt,site);
         			db_query(handler,query);
         			db_clear_result(result);
         			strcpy(reponse,"La catégorie a bien été créée ...");
         			}
					}
				}
			}
