...
categorie=(char*)malloc(query_size);
	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
         	...
         	if(succes==1)
         		{
         		sprintf(query,"select * from categorie where site=\'%s\' and nomcourt=\'%s\';",site,categorie);
					db_query(handler,query);
					if(db_ntuples(result)==0) 
						{
						strcpy(reponse,"Aucune catégorie n'a été selectionnée ...");
						db_clear_result(result);
						}
					else 
						{
						db_clear_result(result);
						strcpy(reponse,"La catégorie a été supprimée ...");
						sprintf(query,"delete from categorie where site=\'%s\' and nomcourt=\'%s\';",site,categorie);
						db_query(handler,query);
						db_clear_result(result);
						sprintf(query,"delete from categorieallouee where site=\'%s\' and categorie=\'%s\';",site,categorie);
						db_query(handler,query);
						db_clear_result(result);
						}
					}
