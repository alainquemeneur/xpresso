         	...
         	if(succes==1)
         		...
         		sprintf(query,"select * from commentaire where site=\'%s\' and article=\'%s\' and pseudo=\'%s\' and horodatage=\'%s\' and valide='non';",site,article,pseudo,horodatage);
					...
					if(db_ntuples(result)==0) 
						...
						{
						db_clear_result(result);
						sprintf(query,"delete from commentaire where site=\'%s\' and article=\'%s\' and pseudo=\'%s\' and horodatage=\'%s\' and valide='non';",site,article,pseudo,horodatage);
						db_query(handler,query);
						strcpy(reponse,"Le commentaire a été supprimé ...");
						db_clear_result(result);
						}
					}
				//db_close(handler);
